...
use crate::core::ports::{
    ...
    Msg, PackageSource,
    PromptSource, SettingsStore, SysIo, ToolRunner, Workspace,
};
use crate::core::prompt::Prompts;
use crate::core::providers::{Channel, ModelEntry, Provider, Settings};
use crate::core::{
    AgentInstance, Core, Live, SessionEvent, WorkMode, WorkSpec,
};
...
pub(crate) fn core_with_workspace(modules: Vec<Module>, gateway: ScriptGateway, ws: Arc<InMemoryWorkspace>) -> Core {
    Core::new(
...
pub(crate) fn core_with_io(modules: Vec<Module>, gateway: ScriptGateway, io: Arc<InMemorySysIo>) -> Core {
    core_with_all(
        modules,
        gateway,
        Arc::new(SilentRunner),
...
pub(crate) fn core_with_runner(modules: Vec<Module>, gateway: ScriptGateway, runner: Arc<impl ToolRunner + Send + Sync + 'static>) -> Core {
