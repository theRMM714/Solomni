...
fn complaint(code: u16, msg: impl Into<String>) -> Reply {
    (code, json_head(), json!({ "error": msg.into() }).to_string())
}
...
fn route(
    ...
    match (method, url) {
            ...
            loop {
                ...
                if !lines.is_empty() || Instant::now() >= deadline {
                    let snap: Vec<serde_json::Value> = lines
                        .iter()
                        .map(|l| json!({ "seq": l.seq, "sid": l.sid, "events": ev_json(&l.events) }))
                        .collect();
        ...
        ("GET", "/api/state") => match state_json(ops, fence) {
            Ok(v) => ok_json(v),
        ...
        ("POST", "/api/sessions") => {
            ...
            let spec = WorkSpec {
                ...
                mode,
                agents,
                task: req.get("task").and_then(|t| t.as_str()).map(|s| s.to_string()),
                delegate: req.get("delegate").and_then(|t| t.as_bool()).unwrap_or(false),
            };
            match ops.sessions.create_work(spec) {
                Ok(o) => ok_json(json!({ "sid": o.sid, "agents": o.agents, "events": ev_json(&o.events) })),
                Err(e) => {
        ...
        ("POST", path) if path.starts_with("/api/sessions/") => {
            ...
            let (sid, action) = match rest.split_once('/') {
                Some(x) => x,
                None => return complaint(404, "路径应为 /api/sessions/{sid}/{action}"),
            };
        ...
        ("POST", "/api/providers") => {
            ...
            match ops.registry.upsert_provider(&str_field(&req, "id"), &str_field(&req, "base_url"), &str_field(&req, "api_key")) {
                Ok(()) => ok_json(json!({ "ok": true })),
                Err(e) => complaint(400, e),
            }
        }
        ("POST", path) if path.starts_with("/api/providers/") => {
            ...
            let (id, action) = match rest.split_once('/') {
                Some(x) => x,
                None => return complaint(404, "路径应为 /api/providers/{id}/{action}"),
            };
            let id = url_decode(id);
            let outcome = match action {
                "remove" => ops.registry.remove_provider(&id).map(|ok| json!({ "ok": ok })),
                "discover" => ops.registry.discover_models(&id).map(|models| json!({ "ok": true, "models": models })),
                _ => Err(format!("未知动作：{}", action)),
            };
            match outcome {
                Ok(v) => ok_json(v),
        ...
        ("POST", path) if path.starts_with("/api/models/") => {
            ...
            let (id, action) = match rest.split_once('/') {
                Some(x) => x,
                None => return complaint(404, "路径应为 /api/models/{id}/{action}"),
            };
            let id = url_decode(id);
            let outcome = match action {
                "remove" => ops.registry.remove_model(&id).map(|ok| json!({ "ok": ok })),
                "core" => ops.registry.set_core_model(&id).map(|ok| json!({ "ok": ok })),
                _ => Err(format!("未知动作：{}", action)),
            };
            match outcome {
                Ok(v) => ok_json(v),
        ...
        ("POST", path) if path.starts_with("/api/agents/") => {
            ...
            let (name, action) = match rest.split_once('/') {
                Some(x) => x,
                None => return complaint(404, "路径应为 /api/agents/{name}/{action}"),
            };
            let name = url_decode(name);
            let outcome = match action {
                "remove" => ops.registry.remove_agent(&name).map(|ok| json!({ "ok": ok })),
                _ => Err(format!("未知动作：{}", action)),
            };
            match outcome {
                Ok(v) => ok_json(v),
        ...
        // ---- 基本设置 ----
        ("GET", "/api/settings") => match ops.registry.settings() {
        ...
        // ---- 会话历史 ----
        ("GET", "/api/history") => match ops.history.list() {
