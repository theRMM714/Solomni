//! Solomni 前端（终端转录中心）：解析命令 → 调核心层 → 渲染事件流。
//! 本文件只做解析与渲染，不做业务决策（决策在 kernel，引擎在 orchestrator）。
...
fn direct_flow(core: &Core, id: &str) {
    let mut s = match core.start_direct(id) {
        Ok(s) => s,
        Err(e) => {
            println!("[错误] {}", e);
            return;
...
fn omni_flow(mut s: kernel::OmniSession) {
    for e in s.open() {
        render(std::slice::from_ref(&e));
    }
