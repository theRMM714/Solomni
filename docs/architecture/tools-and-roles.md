...
**当前差异**：这条公式在**校验**上已经成立（讨论回合与执行回合都查角色表，讨论回合的原生声明槽也用它），
但两处**发放**仍是全量：提示词正文渲染的是整本工具总表，执行回合的原生声明槽也没按角色面过滤。
两处收口记在缺口 `prompt.tool-book-rendered-instead-of-role-face`（`tests/gaps.yaml`）。
