...
| `too_many_arguments` | `core/engine.rs` 的 `core_operation` / `converse_with` / `turn_with`、`core/session.rs` 的 `discussion_turn` | 同一组参数（身份 / 工具面 / 通道 / 消息 / 出口）：它们必须一路透传，收口成参数对象只是把参数挪个地方（`turn_with` 只服务内存测试通道） |
