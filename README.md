...
- **质量门禁（T0）已并入同一入口，且全部零容忍**：编译、结构审查、格式、clippy、编译告警、依赖重复——
  任何一项不过即 `quality-fail`（没有存量基线）。分层、目录、缺口账与报告格式见 [TESTING.md](TESTING.md) 与 `docs/testing/`。

## 跑一遍演示 · Run the Demo

`demo/` 里带着两份可以直接跑的演示：把 5 份混合格式的本地资料（`demo/sample-corpus/`）整理成一份**可离线打开的 HTML 报告**
+ 一个**能毫秒级检索的索引包**。三个模块分别用 **python / node / C++** 写，正好演示"工具的语言完全自由"：
...
# 或者走**小组协作**（三个 agent 各持一个模块，讨论 → 收敛 → 执行 → 验收 → 交付）
node demo/run-demo-collab.mjs
```

| 脚本 | 编排形态 | 验什么 |
|---|---|---|
| `demo/run-demo.mjs` | **组合式**（一个 agent 装三个模块） | 工具语言自由、跨模块数据接力 |
| `demo/run-demo-collab.mjs` | **小组协作**（三个 agent 分权协商） | 协作五阶段真的走完：讨论收敛 → 整理方案 → 各自回报 → 核心验收 → 交付 |

两个脚本都要**真实供应商**（没配供应商会回落到内置假模型：流程照跑，但不会调工具、也不产生产物）。
协作状态机的**机器判据**在 `tests/cross-platform/e2e/`（CI 三平台跑）：agree 收敛 / leave 不可逆 / 轮次上限 / 返工 / 撤回同意 / ask 中止，
以及三种语言的模块在真进程里真的能用（harvest → indexer 建索引 → 检索有命中）。
...
> `docs/architecture/`（模块地图、呈现层入站契约与路由目录、系统工具与角色、任务链与子会话、会话模型与上下文压缩）。完整路由见 [AGENTS.md](AGENTS.md) 的「文档路由」。
