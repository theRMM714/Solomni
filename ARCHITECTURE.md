...
| `EnvelopeRepair` | 手写信封不合法时的**无歧义**补救（改了字段含义就是错；拿不准就返回不修） | `UnambiguousRepair`（转义字符串里的裸控制字符 + 补上扫描器算出的收尾括号；断在字符串中间不修） |
...
| | `tool_texts.*` | **运行时回执**：路径校验、参数不符（说事实 + 回发工具签名）、内置四件套回执与行区间/截断/编码标注、edit 的找不到（含"只差空白"提示）与多处命中、write 的"没读过/读后又被改/只读到一部分"三种拒绝、外部工具分派的三类失败、工具超限、**信封不合法四类**（未闭合"还差什么" / 裸控制字符 / 语法错 / 字段不合法）与"已修复后执行"的如实标注、给模型看的清单骨架、追加在回复行末尾的 `（已停止）` |
